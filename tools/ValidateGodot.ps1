...
$logPath = Join-Path $logDir "godot.log"

& $godot --headless --path $repoRoot --log-file $logPath --quit
...
if ($log -match "SCRIPT ERROR|Parse Error|Compile Error|Failed to load script") {
  throw "Godot headless validation log contains script errors. See $logPath"
}
